...
def test_epoch(epoch, loader, model, criterion):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            count += 1
    print(f"[{epoch}] Test results - Loss: {total_loss/count:.4f}, MSE: {total_mse/count:.12f}, BPP: {total_bpp/count:.12f}")
...
def parse_args(argv):
    ...
    parser.add_argument('--quality', type=int, default=3, help='Model quality level (1-9)') # Tirei de 1-10 porque é só até 9 //Radmann
    ...
    parser.add_argument('--num-frames', type=int, default=5, help='Number of Frames (default 5)')
